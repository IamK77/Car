...
class Dijkstra:
    ...
    def planning(self, sx, sy, gx, gy, sc):  # 起点x   起点y     终点x    终点y
        ...
        while True:
            c_id = min(open_set, key=lambda o: open_set[o].cost)  # 找到open_set中cost最小的元素
...
def draw():
    ...
    ox, oy = [], []
    for i in range(0, 240):  # 绘制零点处(0, 0)至(0, 2400)的围墙
        ox.append(i)
        oy.append(0)
    for i in range(0, 240):  # 绘制零点处(0, 0)至(2400, 0)的围墙
        ox.append(0)
        oy.append(i)
    for i in range(0, 240):  # 绘制(2400, 0)至(2400, 2400)的围墙
        ox.append(i)
        oy.append(240.0)
    for i in range(0, 240):  # 绘制(0, 2400)至(2400, 2400)的围墙
        ox.append(240.0)
        oy.append(i)
    ...
    rx, ry = dijkstra.planning(sx, sy, qrx, qry, "xc")
    rx1, ry1 = dijkstra.planning(qrx, qry, orix, oriy, "xy")
    rx2, ry2 = dijkstra.planning(orix, oriy, frix, friy, "xb")
    rx3, ry3 = dijkstra.planning(frix, friy, halfx, halfy, "xg")
    rx4, ry4 = dijkstra.planning(halfx, halfy, gx, gy, "xc")
    end_time = time.time()
...
# 基于 https://github.com/AtsushiSakai/PythonRobotics 进行改写      如果你们有兴趣可以去看看原版
